...
export class PaginationComponent implements ControlValueAccessor, OnInit {
  ...
  //SMART
  public makeServerCall:boolean = false;
  ...
  protected getPages(currentPage:number, totalPages:number):any[] {
    ...
    if (isMaxSized) {
      ...
      if ((Math.ceil(currentPage / this.maxSize) - 1) == 0) {
        this.makeServerCall = false;
      } else {
        this.makeServerCall = true;
      }
    }
    ...
    this.pageChanged.emit({
      ...
      makeServerCall: this.makeServerCall
    });
    return pages;
  }
